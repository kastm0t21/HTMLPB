<?php include 'header.php' ?>

<div id="all">

    <div id="content">
        <div class="container">

            <div class="col-md-3">
                <!-- *** MENUS AND FILTERS ***
 _________________________________________________________ -->
                <div class="panel panel-default sidebar-menu">

                    <?php include 'sidebar.php' ?>

                </div>


                <!-- *** MENUS AND FILTERS END *** -->
            </div>

            <div class="col-md-9">

                <div class="row" id="productMain">
                    <div class="col-sm-6">
                        <div id="mainImage">
                            <img src="../img/products/bowls/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 64 Ounce, Set of 5, Party Snack or Salad Bowl/61FSWMV8zjL._SL1500_.jpg"
                                alt="" class="img-responsive">
                        </div>

                        <!-- <div class="ribbon sale">
                                <div class="theribbon">SALE</div>
                                <div class="ribbon-background"></div>
                            </div> -->
                        <!-- /.ribbon -->

                        <!-- <div class="ribbon new">
                                <div class="theribbon">NEW</div>
                                <div class="ribbon-background"></div>
                            </div> -->
                        <!-- /.ribbon -->

                    </div>
                    <div class="col-sm-6">
                        <div class="box">
                            <h2 class="text-center">Blue Sky, Square Unbreakable White Plastic Serving Bowls, 64 Ounce, Set of 5, Party Snack or
                                Salad Bowl</h2>
                            <p class="price">$15.02</p>

                            <p class="text-center buttons">
                                <a href="https://www.amazon.com/dp/B016SQ5FN6?th=1" target="_blank" class="btn btn-primary">
                                    <i class="fa fa-shopping-cart"></i> Buy Now!</a>
                            </p>


                        </div>

                        <div class="row" id="thumbs">
                            <div class="col-xs-4">
                                <a href="../img/products/bowls/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 64 Ounce, Set of 5, Party Snack or Salad Bowl/61FSWMV8zjL._SL1500_.jpg"
                                    class="thumb">
                                    <img src="../img/products/bowls/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 64 Ounce, Set of 5, Party Snack or Salad Bowl/61FSWMV8zjL._SL1500_.jpg"
                                        alt="" class="img-responsive">
                                </a>
                            </div>
                            <div class="col-xs-4">
                                <a href="../img/products/bowls/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 64 Ounce, Set of 5, Party Snack or Salad Bowl/717ADXYoAbL._SL1500_.jpg"
                                    class="thumb">
                                    <img src="../img/products/bowls/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 64 Ounce, Set of 5, Party Snack or Salad Bowl/717ADXYoAbL._SL1500_.jpg"
                                        alt="" class="img-responsive">
                                </a>
                            </div>
                        </div>
                    </div>

                </div>


                <div class="box" id="details">
                    <p>
                        <h4>Product details</h4>
                        <ul>
                            <li>Set Of 5 - 64 Oz. Square White Plastic Serving Bowls</li>
                            <li>Durable Plastic Construction, Lightweight for Easy Carrying, Re-Usable/Disposable</li>
                            <li>Modern contemporary design that combines round and square</li>
                            <li>Great for weddings, parties, receptions, patio, poolside and catered events</li>
                            <li>Serving bowl dimensions: 9.5x9.5" Wide, x 3" High</li>
                        </ul>
                        <h4>Other Description</h4>
                        <p>Imagine setting the table of your dreams with dishes that fulfill your desires for beauty as well
                            as quality, create an event with the impressions of real china dishes, but without the hassle
                            of transfer, cleaning and washing! Imagine no more … enter the Party Bargains difference, our
                            collections of Plates, Flatware, Bowls, and stemware will leave your guest breathless, and yourself
                            relieved.
                            <br/>
                            <br/> Add Great Style And Function To Your Dining Or Party Table With These Attractive Beautiful Snack
                            Or Salad Bowls, Make Your Special Event A Success! Durable Plastic Construction, and Lightweight
                            for Easy Carrying, The versatility will compliment any table setting. Elegant enough for formal
                            dinner parties and casual for everyday use. Great for backyard and poolside parties. Does not
                            break when dropped!
                            <br/>
                            <br/> Party Bargains quality products are designed to meet the high demands of commercial foodservice
                            businesses. Every product is independently tested and selected to ensure the performance you
                            expect, for an everyday low price. The result is a quality item backed by our commitment to your
                            satisfaction.
                            <br/>
                            <br/> Party Bargains offers a comprehensive assortment of plastic party and catering tableware. Their
                            designs range from classic to contemporary, and the products establish tomorrow's trends in today's
                            market. A decade of experience in the disposable tableware industry ensures they know just what
                            the customer wants. Party Bargains standards are set high. The commitment to being at the cutting
                            edge of style and originality without ever compromising on quality or practicality is evident
                            in every one of their products, and their mission is to enhance the most upscale event and please
                            the most discriminating customer
                            <br/>
                            <br/> aParty Bargains would like to reassure you that while every dish is disposable our commitment
                            to you is everlasting</p>
                        <h4>Size</h4>
                        <ul>
                            <li>64 ounce</li>
                        </ul>

                        <!-- <blockquote>
                                    <p>
                                        <em></em>
                                    </p>
                                </blockquote> -->
                </div>

                <div>
                    <h4>Related Products:</h4>

                    <div class="col-md-4 col-sm-6">
                        <div class="product">
                            <div class="flip-container">
                                <div class="flipper">
                                    <div class="front">
                                        <a href="../details/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 32 Ounce, Set of 5, Party Snack or Salad Bowl..php">
                                            <img src="../img/products/bowls/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 32 Ounce, Set of 5, Party Snack or Salad Bowl/61gnLd6j-KL._SL1500_.jpg"
                                                alt="" class="img-responsive">
                                        </a>
                                    </div>
                                    <div class="back">
                                        <a href="../details/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 32 Ounce, Set of 5, Party Snack or Salad Bowl..php">
                                            <img src="../img/products/bowls/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 32 Ounce, Set of 5, Party Snack or Salad Bowl/31NhnvTFU7L.jpg"
                                                alt="" class="img-responsive">
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <a href="../details/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 32 Ounce, Set of 5, Party Snack or Salad Bowl..php"
                                class="invisible">
                                <img src="../img/products/bowls/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 32 Ounce, Set of 5, Party Snack or Salad Bowl/61gnLd6j-KL._SL1500_.jpg"
                                    alt="" class="img-responsive">
                            </a>
                            <div class="text">
                                <h5>
                                    <a href="../details/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 32 Ounce, Set of 5, Party Snack or Salad Bowl..php">Blue Sky, Square Unbreakable White Plastic Serving Bowls, 32 Ounce, Set of 5, Party Snack
                                        or Salad Bowl.</a>
                                </h5>
                                <p class="price">$12.59</p>
                                <p class="buttons">
                                    <a href="../details/Blue Sky, Square Unbreakable White Plastic Serving Bowls, 32 Ounce, Set of 5, Party Snack or Salad Bowl..php"
                                        class="btn btn-primary">View Details</a>
                                </p>

                            </div>
                            <!-- /.text -->
                        </div>
                        <!-- /.product -->
                    </div>

                </div>
                <!-- /.product -->

            </div>

        </div>
        <!-- /.col-md-9 -->
    </div>
    <!-- /.container -->
</div>
<!-- /#content -->

<?php include 'footer.php' ?>